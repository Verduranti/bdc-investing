  ...
  try {
    ...
    const universe = bdcs.map(bdc => {
      ...
      return {
        ...
        assetQuality: {
          nonAccrualCostPct: pm.non_accrual_cost_pct ?? 0,
          nonAccrualFVPct: pm.non_accrual_fv_pct ?? 0,
          pikIncomePct: pm.pik_income_pct ?? 0,
          pikIncomePriorQuarterPct: pm.pik_income_prior_pct ?? pm.pik_income_pct ?? 0,
          qoqMarkdownPct: pm.qoq_markdown_pct ?? 0,
          trailingRealizedLossesPct: pm.trailing_realized_losses_pct ?? 0,
          niiPerShare: pm.nii_per_share ?? 0,
          dividendPerShare: pm.dividend_per_share ?? 0,
          dividendCoverage: pm.dividend_coverage ?? 0,
        },
        sectorExposure: {
          software: se?.software_pct ?? 0,
          healthcare: se?.healthcare_pct ?? 0,
          consumer: se?.consumer_pct ?? 0,
          industrial: se?.industrial_pct ?? 0,
          assetBacked: se?.asset_backed_pct ?? 0,
          financial: se?.financial_pct ?? 0,
          other: se?.other_pct ?? 0,
          top10HoldingsPct: se?.top_10_holdings_pct ?? 0,
        },
